...
- **Keep the keyboard working.** macOS hands key events to the most recently registered event tap first, so a tap registered later by the system or another app can silently take the brightness keys away (upstream issue #1868, "opening the settings window fixes it for a while"). The app now re-registers its key tap every minute when idle and shortly after wake or display changes.
- **Brightness keys read directly from the keyboard.** On macOS Tahoe with the lid closed the brightness keys are never turned into key events, so no event tap (and therefore no upstream MonitorControl) can see them, while the volume keys work. The app now additionally reads the keys from the HID layer: through IOHIDManager and through the HID event system (the layer `hidutil monitor` uses, which also sees Bluetooth keyboards on Apple Silicon). Apple keyboards report F1/F2 as plain function keys plus the fn key, so the translation to brightness is done in the app, honouring "Use F1, F2, etc. keys as standard function keys". This needs the *Input Monitoring* permission; macOS asks for it once. Presses that arrive through more than one path are de-duplicated. Confirmed working on a MacBook Air M1 with macOS 26.5, lid closed, Apple Magic Keyboard over Bluetooth.
- **Working on-screen display on macOS 26.** The private OSD framework still shows its bezel on Tahoe but no longer draws the level (upstream issue #1782, declared unfixable by the maintainer). On macOS 26 and newer the app shows its own compact pill in the top right corner of the target display instead, with icon and level bar. Older macOS versions keep the native OSD.
- **A key press never does nothing.** If the display under the mouse pointer cannot be determined (pointer on an edge, stale screen list right after a reconfiguration, no focused window), the keys go to the external displays instead of being dropped.
- **Diagnostics that survive on Tahoe.** Key presses, their target displays and DDC write failures are logged at default level, so `log show --predicate 'process == "MonitorControl"' --last 10m` shows them without `--info`.
...
- **Stable signature.** macOS ties the Accessibility permission, which the keyboard control needs, to the app's signing certificate. Since release 4.3.4-reliability.3 every build is signed with the same self-signed certificate (stored as the repository secrets `MAC_SIGNING_CERT_P12` and `MAC_SIGNING_CERT_PASSWORD`), so the permission survives updates. Coming from an upstream build or from an earlier ad-hoc signed build, grant the permission one more time: remove MonitorControl from *Privacy & Security > Accessibility* and add it back, or run `tccutil reset Accessibility app.monitorcontrol.MonitorControl` and start the app. Without the secrets the workflow falls back to an ad-hoc signature, which changes with every build.
