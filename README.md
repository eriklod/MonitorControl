...
- **Stable signature (optional).** macOS ties the Accessibility permission, which the keyboard control needs, to the app's signing certificate. Builds are ad-hoc signed by default, so after every new build the permission must be granted again (remove MonitorControl from *Privacy & Security > Accessibility* and add it back, or run `tccutil reset Accessibility app.monitorcontrol.MonitorControl`). To avoid that, create a self-signed *Code Signing* certificate in Keychain Access, export it as `.p12`, and add it to the repository as secrets `MAC_SIGNING_CERT_P12` (base64 of the file) and `MAC_SIGNING_CERT_PASSWORD`. Every build is then signed with the same certificate and the permission survives updates.
